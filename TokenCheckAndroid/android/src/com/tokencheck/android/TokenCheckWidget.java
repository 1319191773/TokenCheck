package com.tokencheck.android;
...
public class TokenCheckWidget extends AppWidgetProvider {

    public static final String WIDGET_REFRESH = "com.tokencheck.android.WIDGET_REFRESH";
    public static final String WIDGET_SWITCH = "com.tokencheck.android.WIDGET_SWITCH";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        ComponentName cn = new ComponentName(context, TokenCheckWidget.class);
        int[] ids = mgr.getAppWidgetIds(cn);

        String action = intent.getAction();

        if ("com.tokencheck.android.WIDGET_UPDATE".equals(action)) {
    ...
    private void doApiQuery(Context context) {
        SharedPreferences prefs = context.getSharedPreferences("tokencheck_prefs", Context.MODE_MULTI_PROCESS);
        WidgetDataStore.forceReload(context);
        ...
        new Handler(Looper.getMainLooper()).post(() -> {
            AppWidgetManager mgr = AppWidgetManager.getInstance(context);
            WidgetDataStore.forceReload(context);
            int[] ids = mgr.getAppWidgetIds(new ComponentName(context, TokenCheckWidget.class));
            for (int id : ids) updateAppWidget(context, mgr, id);
        });
    }
    ...
    static void updateAppWidget(Context context, AppWidgetManager mgr, int id) {
        ...
        if (total == 2) {
            ...
            if (cp == 1) {
                ...
                Intent swL = new Intent(context, TokenCheckWidget.class);
            ...
            if (cp == 0) {
                ...
                Intent swR = new Intent(context, TokenCheckWidget.class);
            ...
            Intent twoPrev = new Intent(context, TokenCheckWidget.class);
            ...
            Intent twoNext = new Intent(context, TokenCheckWidget.class);
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (show && idx < total) {
                    ...
                    if (i != 1) {
                        Intent swIntent = new Intent(context, TokenCheckWidget.class);
            ...
            Intent prevIntent = new Intent(context, TokenCheckWidget.class);
            ...
            Intent nextIntent = new Intent(context, TokenCheckWidget.class);
        ...
        Intent ref = new Intent(context, TokenCheckWidget.class);
    ...
    private void scheduleAutoRefresh(Context c) {
        ...
        if (am == null) return;
        int intervalMin = 15;
        SharedPreferences prefs = c.getSharedPreferences("tokencheck_prefs", Context.MODE_MULTI_PROCESS);
        try { intervalMin = Integer.parseInt(prefs.getString("widgetRefreshInterval", "15")); }
        catch (Exception e) { /* use default */ }
        if (intervalMin < 1) intervalMin = 15;
        Intent i = new Intent(c, TokenCheckWidget.class);

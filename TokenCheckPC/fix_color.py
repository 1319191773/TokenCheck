...
# Modify constructor
content = content.replace('setFixedWidth(24);', 'setFixedHeight(24);')
content = content.replace('setMinimumHeight(100);', 'setMinimumWidth(200);')
content = content.replace('setCursor(Qt::SizeVerCursor);', 'setCursor(Qt::SizeHorCursor);')
...
paint_event_new = """void PrecisionTrack::paintEvent(QPaintEvent *)
{
    ...
    QRect trackRect(0, centerY - trackHeight / 2, width(), trackHeight);

    if (m_mode == AlphaMode)
    {
        painter.save();
        QPainterPath clip;
        clip.addRoundedRect(trackRect, trackHeight / 2, trackHeight / 2);
    ...
    int trackRange = maxX - minX;

    qreal fillRatio = m_value / 255.0;
    int thumbX = minX + qRound(trackRange * fillRatio);

    QLinearGradient grad(minX, trackRect.y(), maxX, trackRect.y());
    if (m_mode == ValueMode)
    {
    ...
    qreal ratio = qBound(0.0, (qreal)(thumbX - minX) / trackRange, 1.0);
    QColor borderColor;
    if (m_mode == ValueMode)
        borderColor = QColor::fromRgbF(
            m_baseColor.redF() * ratio,
            m_baseColor.greenF() * ratio,
            m_baseColor.blueF() * ratio);
    else
        borderColor = QColor(m_baseColor.red(), m_baseColor.green(), m_baseColor.blue(),
                             qRound(255 * ratio));

    painter.setPen(QPen(borderColor, 2));
    painter.setBrush(Theme::bg);
    painter.drawEllipse(QPoint(thumbX, centerY), thumbR, thumbR);
}
"""
...
pick_value_new = """void PrecisionTrack::pickValue(const QPoint &pos)
{
    ...
    int minX = margin;
    int maxX = width() - margin;
    qreal ratio = qBound(0.0, (qreal)(pos.x() - minX) / (maxX - minX), 1.0);
...
# Layout modification in setupUI
old_layout_code = """    QHBoxLayout *interactiveLayout = new QHBoxLayout();
...
new_layout_code = """    QVBoxLayout *interactiveLayout = new QVBoxLayout();

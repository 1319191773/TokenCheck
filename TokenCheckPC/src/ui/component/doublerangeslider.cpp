...
void DoubleRangeSlider::paintEvent(QPaintEvent *aEvent)
{
    ...
    if (orientation == Qt::Horizontal)
        backgroundRect = QRectF(scLeftRightMargin, (height() - scSliderBarHeight) / 2.0,
                                width() - scLeftRightMargin * 2, scSliderBarHeight);
    else
        backgroundRect = QRectF((width() - scSliderBarHeight) / 2.0, scLeftRightMargin,
        ...
        if (mFirstHandlePressed && type.testFlag(LeftHandle)) {
            if (posValue - mDelta + scHandleSideLength / 2 <= secondHandleRectPosValue) {
                setLowerValue((posValue - mDelta - scLeftRightMargin - scHandleSideLength / 2) * 1.0
                              / validLength() * mInterval + mMinimum);
            } else {
                setLowerValue(mUpperValue);
            }
        } else if (mSecondHandlePressed && type.testFlag(RightHandle)) {
            if (firstHandleRectPosValue + scHandleSideLength * (type.testFlag(DoubleHandles) ? 1.5 : 0.5)
                <= posValue - mDelta) {
...
void DoubleRangeSlider::mouseReleaseEvent(QMouseEvent *aEvent)
{
    ...
    if (mFirstHandlePressed)
        aHandleStateChangeIndex = 1;
    else if (mSecondHandlePressed)
        aHandleStateChangeIndex = 2;
    mFirstHandlePressed = false;
    mSecondHandlePressed = false;

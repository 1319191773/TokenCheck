...
void PlatformRegistry::init()
{
    ...
    r.registerPlatform(
        {"deepseek", tr("DeepSeek"), "https://api.deepseek.com", "",
         {{"Balance", "balance", true}, {"Granted", "granted", true}}},
        new DeepSeekHandler());
}
                                         ...
                                         PlatformHandler *h)
{
    ...
    e.descriptor = desc;
    e.handler = h;
    m_entries[desc.type] = e;
    m_ownedHandlers.append(h);
}
